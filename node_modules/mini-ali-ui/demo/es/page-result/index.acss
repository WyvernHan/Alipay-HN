/* 全局异常组件背景图片尺寸 */
/* 局部异常组件背景图片尺寸 */
/* 异常组件副标题最小水平间距 */
.am-page-result {
  display: flex;
  flex-direction: column;
  background: #fff;
  background: var(--am-result-background, #fff);
  height: 100vh;
  overflow: hidden;
}
.am-page-result-pic {
  width: 168px;
  height: 168px;
  margin: calc(50vh - 168px) auto 0;
  background-position: center bottom;
  background-repeat: no-repeat;
  background-size: contain;
}
.am-page-result-title {
  margin: 20px auto 0;
  margin: var(--am-result-margin-vertical, 20px) auto 0;
  max-width: calc(100vw - 48px);
  max-width: calc(100vw - var(--am-result-title-h-spacing, 48px));
  overflow: hidden;
  text-overflow: ellipsis;
  white-space: nowrap;
  text-align: center;
  font-size: 18px;
  font-size: var(--am-result-title-fontSize, 18px);
  color: #333;
  color: var(--am-result-title-color, #333);
}
.am-page-result-brief {
  margin: 0 auto;
  padding-top: 12px;
  padding-top: var(--am-result-brief-margin-top, 12px);
  font-size: 13px;
  font-size: var(--am-result-brief-fontSize, 13px);
  color: #999;
  color: var(--am-result-brief-color, #999);
  max-width: calc(100vw - 96px);
  overflow: hidden;
  text-overflow: ellipsis;
  display: -webkit-box;
  -webkit-box-orient: vertical;
  -webkit-line-clamp: 2;
  text-align: center;
}
.am-page-result-button {
  margin: 20px auto 0;
  margin: var(--am-result-margin-vertical, 20px) auto 0;
}
.am-page-result-button-right {
  margin-left: 12px;
  margin-left: var(--am-result-button-margin-left, 12px);
}
.am-page-result .page-network {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*_-bETYhcgLIAAAAAAAAAAABkARQnAQ");
}
.am-page-result .page-error {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*yoWfTbHhZyQAAAAAAAAAAABkARQnAQ");
}
.am-page-result .page-busy {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*WipxRLDnjH8AAAAAAAAAAABkARQnAQ");
}
.am-page-result .page-empty {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*SbMpTZBVQyEAAAAAAAAAAABkARQnAQ");
}
.am-page-result .page-logoff {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*NpTuTqphtXkAAAAAAAAAAABkARQnAQ");
}
.am-page-result .page-payment {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*88yKRKXs7-sAAAAAAAAAAABkARQnAQ");
}
.am-page-result .page-redpacket {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*ihhDTrg9_6cAAAAAAAAAAABkARQnAQ");
}
.am-local-page {
  height: 100%;
  padding-bottom: 20px;
  padding-bottom: var(--am-result-margin-vertical, 20px);
}
.am-local-page .am-page-result-pic {
  width: 100px;
  height: 100px;
  margin: 20px auto 0;
  margin: var(--am-result-margin-vertical, 20px) auto 0;
  background-size: contain;
}
.am-local-page .am-page-result-brief {
  font-size: 15px;
  font-size: var(--am-result-local-brief-fontSize, 15px);
}
.am-local-page .page-empty {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*XqqzSpaXRUcAAAAAAAAAAABkARQnAQ");
}
.am-local-page .page-error {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*oJZsRIB9TGYAAAAAAAAAAABkARQnAQ");
}
.am-local-page .page-network {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*tP6AQa0UANAAAAAAAAAAAABkARQnAQ");
}
.am-local-page .page-busy {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*hkCKTJY-nXMAAAAAAAAAAABkARQnAQ");
}
.am-local-page .page-logoff {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*kButSYpTAYUAAAAAAAAAAABkARQnAQ");
}
.am-local-page .page-payment {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*bp8aTomhW3YAAAAAAAAAAABkARQnAQ");
}
.am-local-page .page-redpacket {
  background-image: url("https://gw.alipayobjects.com/mdn/rms_ce4c6f/afts/img/A*sb4TTqsZRMYAAAAAAAAAAABkARQnAQ");
}
